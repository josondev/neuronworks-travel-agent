...
with st.sidebar:
    ...
    nvidia_api_key = os.environ.get("NVIDIA_API_KEY") or st.text_input("NVIDIA API Key", type="password")
    if not groq_api_key:
        st.warning("Enter GROQ_API_KEY for the semantic router.")
        st.stop()
    if not nvidia_api_key:
        st.warning("Enter NVIDIA_API_KEY for Llama 3.3 70B.")
        st.stop()
    os.environ["GROQ_API_KEY"] = groq_api_key
    os.environ["NVIDIA_API_KEY"] = nvidia_api_key
    st.success("🟢 Connected")
    st.markdown('<div class="muted">Planner: NVIDIA Llama 3.3 70B Instruct · Router: GPT-OSS 20B</div>', unsafe_allow_html=True)
...
def render_trip(trip, plan):
    ...
    if not plan.get("days"):
        ...
        for i,d in enumerate(plan["days"],1):
            ...
            for j,idx in enumerate(d.get("attraction_ids",[])):
            ...
            if i==len(plan["days"]): lines.append("- 🧳 Check-out / departure")
            if not d.get("attraction_ids") and rid is None: lines.append("- Keep this period flexible rather than inventing another attraction.")
            lines.append("")
...
def current_trip_is_valid(trip):
    req=trip.get("request",{}); start,end,today=parse_iso_date(req.get("departDate")),parse_iso_date(req.get("returnDate")),date.today()
...
async def run_agent(chat_history,trip_data,chat_container):
    async with AsyncExitStack() as stack:
        ...
        try:
            ...
            if fresh:
                trip_data.clear(); status.info("🧠 Extracting trip request…"); bundle_tool=next((t for t in tools if t.name=="build_trip_data"),None)
                if not bundle_tool: raise RuntimeError("MCP server does not expose build_trip_data.")
                first=await llm.bind_tools([bundle_tool]).ainvoke([HumanMessage(content=latest_user)])
                ...
                args=dict(call.get("args") or {}); start,end,today=parse_iso_date(args.get("departDate")),parse_iso_date(args.get("returnDate")),date.today()
                ...
                with st.expander("🧰 Live services used",expanded=False): st.caption("Flights · Hotels · Attractions · Restaurants · Weather · Budget"); st.json(args)
                status.info("⚡ Fetching live trip data…"); result=await bundle_tool.coroutine(**args); trip=json.loads(result.content[0].text); trip_data["build_trip_data"]=trip
                valid,error=current_trip_is_valid(trip)
                if not valid: status.empty(); return f"## ⚠️ I can't plan this trip yet\n\n**{error}**"
                status.info("🧠 Planning itinerary with NVIDIA Llama 3.3 70B…"); plan=await build_itinerary_with_llm(llm,trip,latest_user); status.empty(); return render_trip(trip,plan)
            ...
            if isinstance(existing,dict):
                ...
                status.info("♻️ Reusing verified live trip data…"); plan=await build_itinerary_with_llm(llm,existing,latest_user); status.empty(); return render_trip(existing,plan)
